...
mov rcx, [rbp + 8 * 3] ; Number of args on stack
cmp rbx, rcx
je add_nil_to_stack295

adjust_stack_to_list295:
mov rsi, [rbp + 8 * 3]
sub rsi, 1
sub rsi, 1 ; rsi holds the amount of blocks we need to push later
mov rax, SOB_NIL_ADDRESS ; cdr
...
mov rdx, 1
cmp rdx, 0
jne params_length_not_zero295
mov rdx, 1
jmp decided_param_count295
params_length_not_zero295:
mov rdx, rcx
sub rdx, 1
add rdx, 1
decided_param_count295:
mov qword [rbp + 8 * 3], rdx ; updating number of params on stack
dec rcx
add rcx, 4 ; rcx has the offset of the last arg
shl rcx, 3 ; 8*
...
create_list_loop295:
cmp rdi, 0
je end_create_list_loop295
mov rbx, qword [rcx] ; car
MAKE_PAIR(rdx, rbx, rax) ; rdx has new list
mov rax, rdx ; now rax has it ^
sub rcx, 8
dec rdi
jmp create_list_loop295
end_create_list_loop295:
pop rcx ; rcx now points to the last argument on stack
mov qword [rcx], rax ; new list is now in the correct place
; now lets push the stack up
mov rdx, qword [rbp + 8 * 3]
...
push_stack_loop295:
cmp rdi, 0
je end_push_stack_loop295
mov rdx, qword [rbx]
mov qword [rcx], rdx
sub rbx, 8
sub rcx, 8
dec rdi
jmp push_stack_loop295
end_push_stack_loop295:
shl rsi, 3 ; push rbp and rsp up
add rbp, rsi
add rsp, rsi
jmp body295
